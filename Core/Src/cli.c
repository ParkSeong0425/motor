...
static void go(int count, char *word[]) {
	...
	snprintf(cmd, sizeof(cmd), "01MO_1_%lu_%lu_0_0", (unsigned long) num,
			(unsigned long) speed);
	proto(cmd);
}

/* run 명령: 저장된 출고 위치들을 순서대로 왕복 */
static void run_loop(int count, char *word[]) {
	...
	/*
	 * speed is percent, not rpm.
	 *
	 * run 10  -> 10%  -> 100rpm when max rpm is 1000
	 * run 20  -> 20%  -> 200rpm when max rpm is 1000
	 * run 50  -> 50%  -> 500rpm when max rpm is 1000
	 * run 100 -> 100% -> 1000rpm when max rpm is 1000
	 */

	if (count == 2) {
		if (u32(word[1], &speed) == 0 || speed == 0 || speed > 100) {
			printf("run <speed_percent 1~100>\r\n");
			return;
		}

		snprintf(cmd, sizeof(cmd), "01RUN_1_%lu", (unsigned long) speed);
		proto(cmd);
		return;
	}

	if (count >= 3) {
		if (u32(word[1], &num) == 0 || u32(word[2], &speed) == 0 || num == 0
				|| speed == 0 || speed > 100) {
			printf("run <start_save_num> <speed_percent 1~100>\r\n");
			return;
		}

		snprintf(cmd, sizeof(cmd), "01RUN_1_%lu_%lu", (unsigned long) num,
				(unsigned long) speed);
		proto(cmd);
		return;
	}
...
static void run(char *line) {
	...
	if (strcmp(word[0], "help") == 0)
		...
		printf("set=%d\r\n", Motor_Setup(&huart5));
	else if (strcmp(word[0], "wheel") == 0)
		wheel(count, word);
	else if (strcmp(word[0], "zero") == 0)
		proto("01ZERO");
	else if (strcmp(word[0], "home") == 0)
		proto("01HOME");
	else if (strcmp(word[0], "in") == 0)
		inpos(count, word);
	else if (strcmp(word[0], "save") == 0)
		save(count, word);
	else if (strcmp(word[0], "show") == 0)
		proto("01Q");
	else if (strcmp(word[0], "go") == 0)
		go(count, word);
	else if (strcmp(word[0], "run") == 0)
		run_loop(count, word);
	else if (strcmp(word[0], "do28") == 0)
		proto("01D28");
	else if (strcmp(word[0], "power") == 0)
		power(count, word);
		...
		proto("01I");
	else if (strcmp(word[0], "estop") == 0 && count >= 2
			&& strcmp(word[1], "clear") == 0)
		proto("01ECLR");
	else
		printf("?\r\n");
}
