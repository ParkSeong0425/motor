...
/* FRAM CS low/high */
static void cs0(void) { HAL_GPIO_WritePin(FRAM_CS_GPIO_Port, FRAM_CS_Pin, GPIO_PIN_RESET); }
static void cs1(void) { HAL_GPIO_WritePin(FRAM_CS_GPIO_Port, FRAM_CS_Pin, GPIO_PIN_SET); }

/* checksum */
static uint32_t sum_bytes(const uint8_t *p, uint32_t n)
{
    uint32_t s = 0;
    for (uint32_t i = 0; i < n; i++) s = (s * 33u) + p[i];
    return s;
}
...
static uint8_t good(const StRec_t *r)
{
    ...
    if (r->zero_ok > 1u || r->in_ok > 1u) return 0;
...
static uint8_t good_old(const StOldRec_t *r)
{
    ...
    if (r->dia_mm == 0) return 0;
    if (r->in_ok > 1u || r->out_count > ST_MAX) return 0;
...
static void migrate_old(const StOldRec_t *old)
{
    ...
    for (uint32_t i = 0; i < ST_MAX; i++) st.out_pos[i] = old->out_pos[i];
    st.sum = sum(&st);
}
...
HAL_StatusTypeDef St_Save(void)
{
    ...
    if (st.zero_ok > 1u) st.zero_ok = 1u;
...
HAL_StatusTypeDef St_Load(void)
{
    ...
    if (r == HAL_OK && good(&st) != 0) return HAL_OK;
...
/* accessors */
uint32_t St_Dia(void) { return st.dia_mm; }
void St_SetDia(uint32_t dia_mm) { if (dia_mm != 0) st.dia_mm = dia_mm; }

uint8_t St_ZeroOk(void) { return (uint8_t)((st.zero_ok != 0u) ? 1u : 0u); }
int32_t St_Zero(void) { return st.zero_unit; }
...
uint8_t St_InOk(void) { return (uint8_t)((st.in_ok != 0u) ? 1u : 0u); }
int32_t St_In(void) { return st.in_pos; }
...
uint32_t St_Count(void) { return st.out_count; }
