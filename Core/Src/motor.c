...
static void all_off(void) {
    ...
    dox(DO2_PORT, DO2_PIN, 0); dox(DO3_PORT, DO3_PIN, 0); dox(DO4_PORT, DO4_PIN, 0);
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
    ...
    motor_state.error = estop_req; motor_state.moving = 0;
    last = now;
}
...
static uint8_t btn(void) {
    if (estop_req) { motor_state.error = 1; motor_state.moving = 0; return 1; }
    return 0;
}
...
int32_t Motor_mmToUnit(int32_t mm) {
    ...
    if (den <= 0) return 0;
    return (int32_t)(((v * 10000LL * MOTOR_UNIT_PER_TURN * DRIVE_RATIO_NUM) / den) * sign);
}
...
void Motor_InitIO(void) { Bus_Rx(); all_off(); clear_state(); }
void Motor_ClearEStop(void) { estop_req = 0; motor_state.error = 0; dox(DO2_PORT, DO2_PIN, 0); }
uint8_t Motor_IsBusy(void) { return motor_state.moving; }
uint8_t Motor_IsEStop(void) { return btn(); }
uint8_t Motor_IsHold(void) { return hold_on; }
int32_t Motor_HoldPos(void) { return hold_pos; }
void Motor_ClearHold(void) { hold_on = 0; }

/* 위치 읽기 */
HAL_StatusTypeDef Motor_ReadPos(UART_HandleTypeDef *huart, int32_t *pos) {
    if (!huart || !pos) return motor_state.last_result = HAL_ERROR;
    HAL_StatusTypeDef r = Bus_Read32(huart, R_REAL, pos);
    if (r == HAL_OK) motor_state.pos = motor_debug.last_pos = *pos;
...
HAL_StatusTypeDef Motor_Stop(UART_HandleTypeDef *huart) {
    ...
    int32_t p = 0; (void)Motor_ReadPos(huart, &p);
    motor_state.moving = 0; return motor_state.last_result = r;
}
...
HAL_StatusTypeDef Motor_Power(UART_HandleTypeDef *huart, uint8_t on) {
    if (!huart) return fail(HAL_ERROR);
    if (on) {
        if (btn()) return fail(HAL_ERROR);
        if (Bus_Power(huart, 1) == HAL_OK) motor_state.enable = 1;
    } else {
        set_start(huart, 0);
        if (Bus_Power(huart, 0) == HAL_OK) { motor_state.enable = 0; motor_state.setup_ok = 0; motor_state.moving = 0; }
    }
...
HAL_StatusTypeDef Motor_Setup(UART_HandleTypeDef *huart) {
    if (!huart || btn()) return fail(HAL_ERROR);
    Bus_Rx(); HAL_Delay(50);
    if (w16(huart, R_MODE, V_POS) != HAL_OK || set_start(huart, 0) != HAL_OK ||
        w16(huart, R_SRC, V_SRC) != HAL_OK || w16(huart, R_RUN, V_RUN) != HAL_OK ||
        w16(huart, R_END, V_END) != HAL_OK || w16(huart, R_WAIT, 0) != HAL_OK) {
        motor_state.setup_ok = 0; return fail(HAL_ERROR);
    }
    return Motor_Power(huart, 1) == HAL_OK ? (motor_state.setup_ok = 1, motor_state.error = 0, HAL_OK) : fail(HAL_ERROR);
}

/* 모터 주행 시작 파라미터 주입 */
HAL_StatusTypeDef Motor_Start(UART_HandleTypeDef *huart, int32_t target, uint8_t speed, uint16_t acc_ms) {
    if (btn()) return fail(HAL_ERROR);
    if (!motor_state.setup_ok && Motor_Setup(huart) != HAL_OK) return fail(HAL_ERROR);

    uint16_t run_rpm = (speed < LIFT_MIN_PERCENT) ? LIFT_MIN_PERCENT : (speed > LIFT_MAX_PERCENT ? LIFT_MAX_PERCENT : speed);
    run_rpm = (uint16_t)(((uint32_t)LIFT_MAX_RPM * run_rpm) / 100u);

    if (set_start(huart, 0) != HAL_OK) return fail(HAL_ERROR);
    HAL_Delay(50);
    if (w16(huart, R_TYPE, V_ABS) != HAL_OK || w32(huart, R_POS, target) != HAL_OK ||
        w16(huart, R_SPEED, run_rpm) != HAL_OK || w16(huart, R_ACC, acc_ms) != HAL_OK ||
        w16(huart, R_WAIT, 0) != HAL_OK || set_start(huart, 1) != HAL_OK) return fail(HAL_ERROR);
...
HAL_StatusTypeDef Motor_Home(UART_HandleTypeDef *huart) {
    ...
    if (gap < 0) gap = -gap;
    if (gap <= GAP) { motor_state.target = z; motor_state.moving = 0; motor_state.error = 0; return HAL_OK; }

    if (Motor_SendMove(z, 10, 1000, 0, 0) != osOK) return fail(HAL_ERROR);
    motor_state.target = z; motor_state.moving = 1; motor_state.error = 0;
    return HAL_OK;
}

/* OS 메시지 큐 통신 제어 */
static osStatus_t put(uint32_t id, int32_t target, uint32_t speed, uint32_t acc_ms, uint32_t start_ms, uint32_t wait_ms) {
    if (!MotorQueueHandle) return osErrorResource;
    MotorCommand_t cmd = {id, target, speed, acc_ms, start_ms, wait_ms};
    return osMessageQueuePut(MotorQueueHandle, &cmd, 0, 0);
}
osStatus_t Motor_SendMove(int32_t target, uint32_t speed, uint32_t acc_ms, uint32_t start_ms, uint32_t wait_ms) {
    if (!MotorQueueHandle || btn() || motor_state.moving || osMessageQueueGetCount(MotorQueueHandle) > 0) return osErrorResource;
...
osStatus_t Motor_SendStop(void) { return put(MOTOR_CMD_STOP, 0, 0, 0, 0, 0); }
...
static void wait(void) {
    ...
    for (;;) {
        MotorCommand_t cmd;
        if (btn()) { motor_state.moving = 0; motor_state.error = 1; return; }
        if (osMessageQueueGet(MotorQueueHandle, &cmd, NULL, 0) == osOK && cmd.id == MOTOR_CMD_STOP) { (void)Motor_Stop(&huart5); return; }

        if (Motor_ReadPos(&huart5, &now) == HAL_OK) {
            gap = now - motor_state.target; if (gap < 0) gap = -gap;
            if (gap <= GAP) { Motor_ClearHold(); motor_state.moving = 0; motor_state.seq++; return; } // 정상 도착 시 홀드 해제

            if (have) {
                diff = now - last; if (diff < 0) diff = -diff;
                if (diff <= STILL_GAP) {
                    /* 초기 가속구간 1500ms 예외 차단 후 정지 카운트 누적 */
                    if ((HAL_GetTick() - start_tick > 1500u) && (++still >= STILL_CNT)) {
                        set_start(&huart5, 0); // 모터 정지 프레임 송신
                        hold_on = 1; hold_pos = now; // [★핵심 반영] 현재 걸린 위치를 임시 원점으로 박제
                        motor_state.moving = 0; motor_state.seq++; return;
                    }
                } else { still = 0; }
            }
            last = now; have = 1;
...
void Motor_TaskRun(void *argument) {
    MotorCommand_t cmd;
    (void)argument; Motor_InitIO();
    for (;;) {
        if (osMessageQueueGet(MotorQueueHandle, &cmd, NULL, 10) != osOK) { (void)btn(); continue; }
        if (cmd.id == MOTOR_CMD_MOVE) {
            if (cmd.start_ms > 0) osDelay(cmd.start_ms);
            if (Motor_Start(&huart5, cmd.target, (uint8_t)cmd.speed, (uint16_t)cmd.acc_ms) == HAL_OK) wait();
